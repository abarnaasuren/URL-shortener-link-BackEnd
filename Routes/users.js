...
const transporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "abarna.balakrishanan@gmail.com",
...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      const link = `https://abarnaa-url-shortner.netlify.app/activate/${hashedUser._id}?activateToken=${token}`;
      const mailOptions = {
        from: "abarna.balakrishanan@gmail.com",
        to: hashedUser.email,
...
router.post("/activation", async (req, res) => {
  try {
    ...
    if (!user.isActivated) {
      ...
      const link = `https://abarnaa-url-shortner.netlify.app/activate/${user._id}?activateToken=${token}`;
      const mailOptions = {
        from: "abarna.balakrishanan@gmail.com",
        to: user.email,
        subject: "Account Activation link sent",
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    const link = `https://abarnaa-url-shortner.netlify.app/authorize/?id=${user._id}&token=${token}`;
    const mailOptions = {
      from: "abarna.balakrishanan@gmail.com",
      to: user.email,
